...
const inputStyle = {
  ...
  color: "#1E3932",
  boxShadow: "0 1px 3px rgba(30,57,50,0.05), inset 0 1px 0 rgba(255,255,255,0.9)",
};
const onFocus = (e: React.FocusEvent<HTMLInputElement>) => {
  e.target.style.border = "1.5px solid rgba(0,117,74,0.5)";
  e.target.style.boxShadow = "0 0 0 3px rgba(0,117,74,0.10), inset 0 1px 0 rgba(255,255,255,0.9)";
};
const onBlur = (e: React.FocusEvent<HTMLInputElement>) => {
  e.target.style.border = "1px solid rgba(0,98,65,0.12)";
  e.target.style.boxShadow = "0 1px 3px rgba(30,57,50,0.05), inset 0 1px 0 rgba(255,255,255,0.9)";
};
...
export default function RegisterPage() {
  ...
  const strengthColor = ["", "#ef4444", "#f59e0b", "#00754A"][strength];
          ...
          <div className="mt-8 flex items-center gap-3">
            <div className="flex -space-x-2">
              {["AB", "CR", "DW", "EF"].map((initials, i) => (
                <div key={i} className="h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: `hsl(${140 + i * 20}, 30%, 25%)`, border: "2px solid #1E3932", color: "#d4e9e2" }}>
                  {initials[0]}
                </div>
              ))}
            </div>
              ...
              <div className="bezel-inner">
                <div className="p-7">
                  ...
                  <div className="mb-6">
                    ...
                    <h1 className="text-2xl font-bold tracking-tight leading-tight mb-1.5" style={{ color: "#1E3932" }}>Buat akun baru</h1>
                    <p className="text-sm" style={{ color: "#5a5550" }}>
                      Sudah punya akun?{" "}
                      <Link href="/login" className="font-semibold hover:underline" style={{ color: "#006241" }}>Masuk di sini</Link>
                    </p>
                  </div>
                  ...
                  {success && (
                    <div className="mb-4 flex items-start gap-2.5 p-3.5 rounded-2xl text-sm" style={{ background: "rgba(0,117,74,0.06)", border: "1px solid rgba(0,117,74,0.18)", color: "#1E3932" }}>
                      <CheckCircle2 className="h-4 w-4 mt-0.5 shrink-0" strokeWidth={1.5} style={{ color: "#00754A" }} />
                      <span>{success}</span>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      {formData.confirmPassword.length > 0 && (
                        <div className="flex items-center gap-1.5 mt-1 text-xs" style={{ color: formData.password === formData.confirmPassword ? "#00754A" : "#ef4444" }}>
                  ...
                  {/* Feature hints */}
                  <div className="mt-6 pt-5" style={{ borderTop: "1px solid rgba(30,57,50,0.05)" }}>
                    <div className="grid grid-cols-4 gap-2">
                      {[{ Icon: ScanLine, l: "Scan" }, { Icon: ChefHat, l: "Resep AI" }, { Icon: Clock, l: "Reminder" }, { Icon: Leaf, l: "Zero Waste" }].map(({ Icon, l }) => (
                        <div key={l} className="flex flex-col items-center gap-1.5 p-2.5 rounded-xl text-center" style={{ background: "rgba(45,106,79,0.05)", border: "1px solid rgba(45,106,79,0.1)" }}>
                           <Icon className="h-3.5 w-3.5" style={{ color: "#00754A" }} strokeWidth={1.5} />
